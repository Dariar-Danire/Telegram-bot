...
func main() {
	...
	router.HandleFunc("/GetUsers", logging(GetUsers))                       //Получение списка пользователей
	router.HandleFunc("/Oauth/redirect", logging(oauthHandler))             // Это для гитхаба!
	router.HandleFunc("/ChangeRoles", logging(ChangeRoles))                 // Запрос на изменение ролей
...
func rightsHendler(w http.ResponseWriter, r *http.Request) {
	...
	if Roles[0] == "Administrator" || Roles[1] == "Administrator" || Roles[2] == "Administrator" || actionCode == "changeName" || actionCode == "changeGroup" || actionCode == "Exit" || actionCode == "Enter" {
		res = true
	}
...
func aouth1Hendler(w http.ResponseWriter, r *http.Request) {
	...
	UserAuthanticate.Time = time.Now().Add(10 * time.Minute)
...
func oauthHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Можно ли не заморачиваться над тем, чтобы могли одновременно работать несколько пользователей
...
func NewUser(NewUser User) map[int]User {
...
func CheckAccessForAdmin(w http.ResponseWriter, r *http.Request) {
	...
	user, ok := Users[GitHub_id]
	if !ok {
		fmt.Println("Error: Такого пользователя нет")
		w.Write([]byte("401"))
		return
	}
	Roles := user.Roles
